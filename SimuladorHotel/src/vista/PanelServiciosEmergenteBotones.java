...
public class PanelServiciosEmergenteBotones extends JPanel {
	...
	public PanelServiciosEmergenteBotones(MicroControladorLayersPadreHijo microControlador, String padre, Controlador controlador, Semaphore s) {
		this.s = s;
		...
		JLabel label = new JLabel(String.valueOf(controlador.getServicios().getBotones().getPrecio()) + " €");
		panel.add(label);
					...
					public void run() {
						try {
							...
							mostrarPanelConfirmacion(
									String.valueOf(controlador.getServicios().getBotones().getPrecio()) + " €");
